...
resource "null_resource" "databricks_account_scim_token" {
  ...
  provisioner "local-exec" {
    ...
    command     = "pwsh ./manage-synapse-mpes.sh -DatabricksAccountId ${var.databricks_account_id} -KeyVaultName ${module.key_vault_scim.key_vault_name} -KeyVaultSecretName ${local.databricks_account_scim_secret_name}"
    environment = {}
  }
